...
const CommentList = ({ student, studentsScores }) => {
  const [comment, setComment] = useState(student.comments.classmaster);
  ...
  const percentage = (
    ...
    ) / Number(scored[0].length)
  ).toFixed(2);
  useEffect(() => {});

  const _commentResult = (comment, total, percentage) => {
    setLoading(true);
    if (comment != "") {
      const payload = {
        classmaster: comment,
  ...
  return (
    <>
      ...
      {loading === true ? (
        ...
        <>
          {student.comments.classmaster !== null  ? (
            <>
              <textarea
                value={comment}
                onChange={(e) => {
      ...
      >
        <Button
          onClick={() => {
            _commentResult(comment, total, percentage);
          }}
        >
          Comment
        </Button>
